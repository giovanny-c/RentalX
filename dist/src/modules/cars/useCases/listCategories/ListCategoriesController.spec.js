...
describe("List Categories", () => {
    ...
    it("Should be able list all categories", () => __awaiter(void 0, void 0, void 0, function* () {
        const responseToken = yield request(app_1.app).post("/sessions").send({
            email: "admim@rentalx.com.br",
            password: "admin"
        });
        const { refresh_token } = responseToken.body;
        yield request(app_1.app).post("/categories").send({
            name: "test category",
            description: "Category test"
        }).set({
            Authorization: `Bearer ${refresh_token}`
        });
        const response = yield request(app_1.app).get("/categories");
