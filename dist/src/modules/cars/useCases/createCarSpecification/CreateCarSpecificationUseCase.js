...
let CreateCarSpecificationUseCase = class CreateCarSpecificationUseCase {
    ...
    execute({ car_id, specifications_ids }) {
        return __awaiter(this, void 0, void 0, function* () {
            const carExists = yield this.carsRepository.findById(car_id);
            if (!carExists) {
                throw new AppError_1.AppError("Car does not exists!");
            }
            const specifications = yield this.specificationsRepository.findByIds(specifications_ids);
            ...
            yield this.carsRepository.create(carExists); // vai fazer um update de cars
            return carExists;
        });
    }
};
