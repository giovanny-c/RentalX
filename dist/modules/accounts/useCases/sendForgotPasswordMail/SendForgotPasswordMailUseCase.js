...
let SendForgotPasswordMailUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute(email) {
    ...
    const user = await this.userRepository.findByEmail(email); //pegando o template de email passando o path
    ...
    const variables = {
      name: user.name,
      link: `${process.env.FORGOT_MAIL_URL}${token}` //ver .env na raiz do projeto
